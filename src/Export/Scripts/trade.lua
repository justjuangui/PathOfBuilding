...
launch:DownloadPage("https://www.pathofexile.com/trade/search", function(response, errMsg)
	if errMsg then
		return "POE ERROR", "Error: "..errMsg
	end

	-- we are going to find in the response the reuire.config({})
	local configStr = response.body:match('require%.config%(%{(.+)%}%);')
	if not configStr then
		print("Failed to Get require.config")
		return
	end

	local pathUrl = configStr:match('baseUrl: (%b"")'):gsub('"', '')
	local tradeSetting = configStr:match('"trade":(%b"")'):gsub('"', '')

	local tradeUrl = pathUrl .. tradeSetting .. '.js'

	launch:DownloadPage(tradeUrl, function(response, errMsg)
		if errMsg then
			print("POE ERROR", "Error: "..errMsg)
			return
		end

		local tradeData = response.body:match('options:(%b[])')
		if not tradeData then
			print("Failed to Get PoE trade data from " .. tradeUrl)
			return
		end
		
		tradeData = tradeData:gsub('e%.translate%((%b"")%)', '%1')
		tradeData = tradeData:gsub('id', '"id"'):gsub('text', '"text"')
		local data, _, err = dkjson.decode(tradeData)
		if err then
			print("Failed to parse trade data")
			return
		end

		local out = io.open("../Data/TradeItemCategoryParser.lua", "w")
		out:write('-- This file is automatically generated, do not edit!\n')
		out:write('-- Trade data (c) Grinding Gear Games\n\nreturn {\n')

		local excludeFilterItems = {"map", "heistequipment", "heistmission", "sanctum", "currency", "logbook", "corpse", "monster", "card", "memoryline", "leaguestone"}
		for _, itemFilter in ipairs(data) do
			-- we are going to filter the current id isnt in exluce list
			if not itemFilter.id then
				goto continue
			end
			for _, excludeFilter in ipairs(excludeFilterItems) do
				if itemFilter.id:lower():find(excludeFilter) then
					goto continue
				end
			end

			out:write('\t["'..itemFilter.text..'"] = "'..itemFilter.id..'",\n')
		...
		printScopeInfo("Item filter info downloaded")
	end)
end)