	...
	run = function (item, modTrade, parseLineTrade, parseRangeOrCustom)
		if item.enchantModLines and #item.enchantModLines > 0 then
			...
			for _, mod in ipairs(item.enchantModLines) do
				...
				local displayName = colorCode .. ((mod.crafted and "Crafted") or (mod.custom and "Custom") or (mod.scourge and "Scourge") or (mod.fractured and "Fractured") or (mod.cricible and "Crucible") or "Enchant")
